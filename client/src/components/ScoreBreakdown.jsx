...
function ScoreBreakdown({ fitScore, similarity, topMatches, matchedSkills, missingSkills }) {
  ...
  return (
    <div className="rounded-ui border border-ink-200">
      ...
      {isOpen && (
        <div className="space-y-6 border-t border-ink-200 p-4">
          {/* ---- 1. The arithmetic ---- */}
          <div>
            ...
            {pairs.length > 0 ? (
              <ul className="mt-2 space-y-3">
                {pairs.map((pair, index) => (
                  <li key={index} className="rounded-ui border border-ink-200 bg-ink-50 p-3">
                    <div className="flex items-baseline justify-between gap-3">
                      <span className="text-label uppercase text-ink-400">
                        Pair {index + 1}
                      </span>
                      <span className="text-meta font-semibold tabular-nums text-ink-600">
                        {pair.similarity.toFixed(3)}
                      </span>
                    </div>

                    <p className="mt-2 text-meta text-ink-500">Your resume</p>
                    <p className="text-body text-ink-800">&ldquo;{pair.resume}&rdquo;</p>

                    <p className="mt-2 text-meta text-ink-500">The posting</p>
                    <p className="text-body text-ink-800">&ldquo;{pair.job}&rdquo;</p>
                  </li>
                ))}
              </ul>
            ) : (
          ...
          <div>
            ...
            <div className="mt-3 grid gap-4 sm:grid-cols-2">
              <div>
                <p className="text-meta font-medium text-ink-700">In both</p>
              ...
              <div>
                <p className="text-meta font-medium text-ink-700">Asked for, not found</p>
                <div className="mt-1.5">
                  <SkillTags
